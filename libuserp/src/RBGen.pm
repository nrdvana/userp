...
sub node_type     { $_[0]{node_type} || $_[0]->namespace.'RBNode_t' }
...
sub write_api {
	...
	my $api= 'extern';
	my $src= <<"END";
/* Auto-Generated by ${\ref $self} version ${\$self->VERSION} on ${\scalar localtime} */
 ...
 */
typedef struct ${ns}RBNode_s {
	struct ${ns}RBNode_s *left, *right, *parent;
	size_t color: 1;
#if SIZE_MAX == 0xFFFF
	size_t count: 15;
...
bool ${ns}node_search(
	...
	if (result_first || result_count) {
		...
		while (NOT_SENTINEL(test)) {
			cmp= compare( goal, PTR_OFS(test,cmp_ptr_ofs) );
			if (cmp == 0) {
				first= test;
...
void ${ns}node_insert( $nd *hint, $nd *node, int(*compare)(void *a, void *b), int cmp_ptr_ofs) {
	...
	while (1) {
		cmp= compare( PTR_OFS(node,cmp_ptr_ofs), PTR_OFS(pos,cmp_ptr_ofs) );
		if (cmp < 0) {
			rightmost= false;
			next= pos->left;
		} else {
	...
	if (NOT_ROOTSENTINEL(hint->parent) && (cmp < 0? leftmost : rightmost)) {
		...
		while (1) {
			if ((cmp < 0? parent->right : parent->left) == hint) {
				if ((cmp < 0) == (compare(PTR_OFS(node,cmp_ptr_ofs), PTR_OFS(parent,cmp_ptr_ofs)) < 0)) {
...
int ${ns}check_structure($nd *node, int(*compare)(void *a, void *b), int cmp_pointer_ofs) {
	...
	if (node && node->parent && !node->parent->parent) {
		if (IS_RED(node) || IS_RED(node->parent) || node->parent->left != node || !node->parent->right || GET_COUNT(node->parent))
			return RBTREE_INVALID_ROOT;
		if (GET_COUNT(node->parent->right) || node->parent->right->left != node->parent->right
...
sub write_wrapper {
	...
	my $tree_t= $opts{tree_type} || do { (my $x= $obj_t.'_'.$node) =~ s/_t(_|$)/$1/g; $x; };
